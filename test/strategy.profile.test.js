...
describe('Strategy#userProfile', function () {
	...
	describe('fetched from default endpoint', function () {
		...
		before(function (done) {
			strategy.userProfile('token', { user_id: '6253282' }, function (err, p) {
	...
	describe('skipping extended profile', function () {
		...
		before(function (done) {
			strategy.userProfile('token', { "user_id": "1705", "screen_name": "jaredhanson" }, function (err, p) {
				if (err) { return done(err); }
				profile = p;
				done();
			});
	...
	describe('error caused by invalid token', function () {
		...
		before(function (done) {
			strategy.userProfile('x-token', { user_id: '123' }, function (e, p) {
				err = e;
				profile = p;
				done();
			});
	...
	describe('error caused by malformed response', function () {
		...
		before(function (done) {
			strategy.userProfile('token', { user_id: '123' }, function (e, p) {
				err = e;
				profile = p;
				done();
			});
